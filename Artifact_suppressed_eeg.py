...
def read_data(file_path):
    ...
    lower_limit = mean_entropy - 5*margin_of_error  # Lower limit for thresholding
    #if lower_limit==nan:
    ...
    upper_limit = mean_kurtosis + 5*margin_of_error  # Upper limit for thresholding
    # Identify ICs with mMSE below the lower limit
    ...
    level = 4
    def custom_threshold(data, level, method='soft'):
        ...
        threshold_method = 'soft'  # Choose 'hard' or 'soft'

    def wavelet_denoising(x, wavelet='bior4.4', level=level, method='soft'):
...
np.save('artifact_suppression.npy', data_array)
# Load the saved stacked_array
np.save('label_artifact_suppression.npy', label_array)

# Load the saved stacked_array
data_array = np.load('artifact_suppression.npy')
# Load the saved stacked_array
label_array = np.load('label_artifact_suppression.npy')
...
# Assuming input_shape is a tuple (height, width, channels)
...
plt.savefig('1dcnn_artifact_suppresed_u.png')
plt.show()
plt.close()
